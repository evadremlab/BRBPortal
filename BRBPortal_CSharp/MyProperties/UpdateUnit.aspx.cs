            ...
            try
            {
                ...
                if (provider.UpdateUnit(ref user))
                {
                    if (user.Cart.Items.FindAll(x => x.PropertyId == user.CurrentProperty.PropertyID).Count > 0)
                    {
                        ...
                        Master.ShowOKModal("Updated Unit Status. Fees may have changed as a result of this update, so please review and add this property to your cart again.", "Update Unit Status");
                    }
                    else
                    {
