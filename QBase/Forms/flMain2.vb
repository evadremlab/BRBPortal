...
#End Region
    ...
    Public Class flMain2
        ...
        'Public iVersionTitle As String = ""
        ...
        Protected Event OnShowSQL() 'SDC 09/08/2014
...
#Region "---------------------------- Load ------------------------------"
        ...
        Private Sub flMain_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            Try
                ...
                ' GBV 4/24/2014 - we need the connection string to open the form in the designer
                If gSQLConnStr = "" Then gSQLConnStr = "Data Source=D-OTT-SQL01\IAS;Initial Catalog=SQLPTS;Connection Timeout=30;Persist Security Info=True;User ID=ptswriter;Password=ptswriter"

                ''...pass title of default version
                'Dim DefaultTitle As String = SQLGetString("Select Title From tRpt Where ClassName = '" & GetObjName() & _
                '                                "' And SavedBy = '" & gUserName & "' And DefaultQuery = 'True'")
                'If DefaultTitle = "" Then DefaultTitle = "Query"
                'FillQBEDefaults(DefaultTitle)
                ''...save currently-loaded version
                'iVersionTitle = DefaultTitle

                '...pass recno of default version
...
#Region "---------------------------- Command Buttons and Menu  ------------------------------"
        ...
        Private Sub tsSaveQuery_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsSaveQuery.Click
            Try
                ...
                Dim Cn As New SqlClient.SqlConnection(gSQLProdConnStr)
                Dim classnamedescr As String = SQLGetString("Select menuname From gmenu Where resname = '" & resname & "'", Cn)
                ...
                'frm.iClassname = objname
                'frm.iClassnamedescr = classnamedescr
                ''NOTE: iAllowEmails is set in flMain2, which may be set in the function's List program
                'frm.iAllowEmails = iAllowEmails
                'frm.iQBEControl = Me
                'frm.iOriginalRecno = iVersionRecno
                'frm.iVersionType = "S"
                'frm.StartPosition = System.Windows.Forms.FormStartPosition.CenterParent
                'frm.ShowDialog()
                SetMenu() ' GBV 2/2/2015
        ...
        Private Sub tsLoadQuery_Click(sender As System.Object, e As System.EventArgs) Handles tsLoadQuery.Click
            Try
                ...
                'NOTE: ALWAYS load the query when returning from the Load Query screen (01/30/2015 SDC)
        ...
        Private Sub tsFieldChooser_Click(sender As Object, e As EventArgs) Handles tsFieldChooser.Click

            'Check if there are any GridViews on form
            If iGVs.Count < 1 Then Return

            If iGVs.Count > 2 Then
                MsgBox("Screen has more than 2 gridviews. Please contact QSI.", MsgBoxStyle.OkOnly, "Field Chooser")
                Return
            End If

            Dim tListNV As New ArrayList        'Non-Visible list
            Dim tListV As New ArrayList         'Visible list
        ...
        Overrides Function ReceiveMenuClick(ByVal aMenuItemName As String, ByVal aMenuItemTag As String) As Boolean
            ...
            If aMenuItemTag.ToLower = "tssavequery" Then tsSaveQuery.PerformClick()
            If aMenuItemTag.ToLower = "tsloadquery" Then tsLoadQuery.PerformClick()
            If aMenuItemTag.ToLower = "tsfieldchooser" Then tsFieldChooser.PerformClick()
            SetMenu()
        End Function
        ...
        '''<summary> Show or hide Query Save button in toolbar </summary>
        Sub ShowFldChooser(ByVal aOK As Boolean)
            tsFieldChooser.Visible = aOK
        End Sub
...
#Region "---------------------------- Link To Edit Form  ------------------------------"
        ...
        Overrides Function QueryAfterNewRecordSave() As Boolean
            ...
            'GBV 6/15/2015 - Added check to iListSQL in case a newly added record should be on a list form with no records selected
            If iListGV IsNot Nothing Then
                If iListGV.RowCount < 1 AndAlso iListSQL = "" Then Return False
            End If
            ...
            If iEditForm IsNot Nothing AndAlso iListSQL <> "" AndAlso iListGV IsNot Nothing Then
                Dim SQL As String = iListSQL
                If iEditForm.iKeyFields.Count > 0 Then
                    Dim PickOneRow As Boolean = True

                    ' Pick up the fieldnames and values
                    Dim C As Object = Nothing
                    For i As Integer = 0 To iEditForm.iKeyFields.Count - 1
                        C = iEditForm.iKeyFields.Item(i)
                        If TypeOf (C) Is qTextBox Then
                            Dim TB As qTextBox = CType(C, qTextBox)
                            If TB.Text <> "" AndAlso TB._BindDef <> "" Then
                                Dim Type As String = "str"
                                If TB._DataType = DataTypeEnum.Dat Then Type = "dat"
                                If TB._DataType = DataTypeEnum.Num Then Type = "num"
                                SQL = AddWhere(SQL, TB._BindDef, TB.Text, Type)
                            Else
                                Query()
                                PickOneRow = False
                                Exit For
                            End If
                        ElseIf TypeOf (C) Is qComboBox Then
                            Dim CB As qComboBox = CType(C, qComboBox)
                            Dim Value As String = ""
                            ...
                            Else
                                Value = CB.Text
                            End If
                            If Value <> "" AndAlso CB._BindDef <> "" AndAlso Not CB._DataType = Nothing Then
                                Dim Type As String = "str"
                                If CB._DataType = DataTypeEnum.Dat Then Type = "dat"
                                If CB._DataType = DataTypeEnum.Num Then Type = "num"
                                SQL = AddWhere(SQL, CB._BindDef, Value, Type)
                            Else
                                Query()
                                PickOneRow = False
                                Exit For
                            End If
                        ElseIf TypeOf (C) Is qDD Then
                            Dim DD As qDD = CType(C, qDD)
                            Dim Value As String = ""
                            If DD._MustMatchList Then
                            ...
                            Else
                                Value = DD.Text
                            End If
                            If Value <> "" AndAlso DD._BindDef <> "" AndAlso Not DD._DataType = Nothing Then
                                Dim Type As String = "str"
                                If DD._DataType = DataTypeEnum.Dat Then Type = "dat"
                                If DD._DataType = DataTypeEnum.Num Then Type = "num"
                                SQL = AddWhere(SQL, DD._BindDef, Value, Type)
                            Else
                                Query()
                                PickOneRow = False
                                Exit For
                    ...
                    ' There are a million things that can go wrong, so better trap errors and call Query()
                    Try
                        If PickOneRow Then
                            Dim DV As DataView = SQLBuildDV(SQL, False)

                            ' If no rows, the new row does not match the query criteria, and should not
                            ' be added to the list.
                            If DV.Count = 0 Then
                                MsgBoxInfo("The new record does not match the query criteria in the list form, " & _
                                           "so it will not be added to it.")
                                Return True
                            End If
                            ' Insert row into iListGV
                            If TypeOf (iListGV.DataSource) Is BindingSource Then
                                Dim R As DataRow = CType(CType(iListGV.DataSource, BindingSource).DataSource, DataView).Table.NewRow
                                R.ItemArray = DV.Table.Rows(0).ItemArray
                                CType(CType(iListGV.DataSource, BindingSource).DataSource, DataView).Table.Rows.Add(R)
                            ElseIf TypeOf (iListGV.DataSource) Is DataView Then
                                Dim R As DataRow = CType(iListGV.DataSource, DataView).Table.NewRow
                                R.ItemArray = DV.Table.Rows(0).ItemArray
                                CType(iListGV.DataSource, DataView).Table.Rows.Add(R)
                            ElseIf TypeOf (iListGV.DataSource) Is DataTable Then
                                Dim R As DataRow = CType(iListGV.DataSource, DataTable).NewRow
                                R.ItemArray = DV.Table.Rows(0).ItemArray
                                CType(iListGV.DataSource, DataTable).Rows.Add(R)
                            Else
                                Query()
                            End If
...
#Region "-------------------- Buttons and Form Close ----------------------------"
        ...
        'GBV 7/21/2014 - So we can export from crons
        Public Sub Export()
            btnExport1.PerformClick()
        End Sub
